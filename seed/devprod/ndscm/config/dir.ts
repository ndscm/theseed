 ...
 * | **tidy**        | git-tracked  | Regenerate checked-in files (`go.sum`, `BUILD.bazel`, locks).      |
 ...
 * | **test:tidy**   | *(none)*     | Run heavy tidy tests.                                              |
 *
 * @example
 * ```ts
 * export default {
...
export type DirConfig = {
  ...
  /**
   * Regenerate **git-tracked** files (`go.sum`, `BUILD.bazel`, lock files)
   * to keep them in sync with their sources.
   *
   ...
   *
   * Two paths: **bazel** builds depend on vendor; **community** builds
   ...
   * artifacts.
   *
   * Three paths: **bazel** tests follow a bazel build, **community** tests
   * follow a community build, and **tidy** tests follow tidy.
   *
   * @example
   * ```ts
   * test: {
