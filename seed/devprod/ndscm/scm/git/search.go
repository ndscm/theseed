...
func SearchTrailer(gitDir string, trailerKey string, text string) (string, error) {
	gitArgs := []string{}
	if gitDir != "" {
		gitArgs = append(gitArgs, "--git-dir", gitDir)
	}
	gitArgs = append(gitArgs, "log", "--format=%H%x00%(trailers:key="+trailerKey+",valueonly,separator=%x01)", "HEAD")
