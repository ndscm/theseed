...
A consumer that loads the image under a _different_, single-uid user namespace —
e.g. a nested rootless `podman load` — cannot map that `rootid`, so applying the
xattr fails the whole load with:
