...
EOF
  if ! grep -F -x 'source $HOME/.seed_managed_profile' ${HOME}/.profile; then
    printf '\nsource $HOME/.seed_managed_profile\n' >>${HOME}/.profile
  fi
  if ! grep -F -x 'source $HOME/.seed_managed_profile' ${HOME}/.bash_profile; then
    printf '\nsource $HOME/.seed_managed_profile\n' >>${HOME}/.bash_profile
  fi
  if ! grep -F -x 'source $HOME/.seed_managed_profile' ${HOME}/.zprofile; then
    printf '\nsource $HOME/.seed_managed_profile\n' >>${HOME}/.zprofile
  fi
  if ! grep -F -x 'source $HOME/.seed_managed_shrc' ${HOME}/.bashrc; then
    printf '\nsource $HOME/.seed_managed_shrc\n' >>${HOME}/.bashrc
  fi
  if ! grep -F -x 'source $HOME/.seed_managed_shrc' ${HOME}/.zshrc; then
    printf '\nsource $HOME/.seed_managed_shrc\n' >>${HOME}/.zshrc
  fi
fi
