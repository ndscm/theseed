...
func (svc *HooinDictateService) SubscribeBrainStep(
	...
	// already registered, so steps emitted during the backfill are buffered on
	// `merged` (not lost) and delivered right after; steps that appear in both
	// the persisted backlog and the live buffer are deduplicated on the client
	// by step uuid. If `since` is unset there is nothing to backfill and we
	// serve live steps only.
...
// sendBackfillSteps streams every persisted BrainStep emitted strictly after
...
func (svc *HooinDictateService) sendBackfillSteps(
	...
	filters := []*sql.Predicate{sql.GT(brainstep.FieldEmitTime, since)}
